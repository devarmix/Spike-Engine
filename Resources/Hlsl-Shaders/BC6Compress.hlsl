...
// SHADER CODE
[[vk::binding(0, 0)]] TextureCube SrcTexture;
	...
	uint3 groupThreadID : SV_GroupThreadID)
{
	...
	if (all(blockCoord < block_dim))
	{
        ...
        float3 block0UV = UvToCubemap(uv, dispatchThreadID.z);
		float3 block1UV = UvToCubemap(uv + float2(2.0f * TextureSizeRcp.x, 0.0f), dispatchThreadID.z);
		float3 block2UV = UvToCubemap(uv + float2(0.0f, 2.0f * TextureSizeRcp.y), dispatchThreadID.z);
		float3 block3UV = UvToCubemap(uv + float2(2.0f * TextureSizeRcp.x, 2.0f * TextureSizeRcp.y), dispatchThreadID.z);
		float4 block0X = SrcTexture.GatherRed(TexSampler, block0UV);
		float4 block1X = SrcTexture.GatherRed(TexSampler, block1UV);
		float4 block2X = SrcTexture.GatherRed(TexSampler, block2UV);
		float4 block3X = SrcTexture.GatherRed(TexSampler, block3UV);
		float4 block0Y = SrcTexture.GatherGreen(TexSampler, block0UV);
		float4 block1Y = SrcTexture.GatherGreen(TexSampler, block1UV);
		float4 block2Y = SrcTexture.GatherGreen(TexSampler, block2UV);
		float4 block3Y = SrcTexture.GatherGreen(TexSampler, block3UV);
		float4 block0Z = SrcTexture.GatherBlue(TexSampler, block0UV);
		float4 block1Z = SrcTexture.GatherBlue(TexSampler, block1UV);
		float4 block2Z = SrcTexture.GatherBlue(TexSampler, block2UV);
		float4 block3Z = SrcTexture.GatherBlue(TexSampler, block3UV);
